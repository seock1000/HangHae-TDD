...
class ChargePointDtoTest {
     ...
     */
    @Test
    @DisplayName("ChangePointDto 생성 시, 유저 ID와 사용액이 1이상이면 충전에 성공한다.")
    void createChargePoint() {
        ...
        //when & then
        new ChargePointDto(userId, amount);
    }
     ...
     * 사용액이 형식상 옳지 않으면 예외를 반환합니다.
     */
    @Test
    @DisplayName("ChargePointDto 생성 시, 사용액이 1 미만이면 IllegalArgumentException을 throw 한다.")
    void invalidAmount() {
        //given
        long userId = 1L;
        long amount = 0L;
