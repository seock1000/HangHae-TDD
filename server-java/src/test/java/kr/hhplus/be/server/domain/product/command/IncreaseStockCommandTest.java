...
class IncreaseStockCommandTest {

    @Test
    @DisplayName("IncreaseStockCommand 생성 시, 상품 ID와 수량이 0보다 크면 객체가 생성된다.")
    void testIncreaseStockCommand() {
        // given
        Long productId = 1L;
        int amount = 10;
    ...
    @Test
    @DisplayName("IncreaseStockCommand 생성 시, 상품 ID가 0 이하이면 InvalidProductError 예외가 발생한다.")
    void testInvalidProductId() {
        ...
        Exception exception = assertThrows(InvalidProductError.class, () -> {
            new IncreaseStockCommand(invalidProductId, amount);
        });
        assertEquals("잘못된 상품 ID 형식입니다.", exception.getMessage());
    }

    @Test
    @DisplayName("IncreaseStockCommand 생성 시, 수량이 0 이하이면 InvalidAmountError 예외가 발생한다.")
    void testInvalidAmount() {
        ...
        Exception exception = assertThrows(InvalidAmountError.class, () -> {
            new IncreaseStockCommand(productId, invalidAmount);
        });
