...
class UsePointCommandTest {
    ...
    @Test
    @DisplayName("UsePointCommand 생성 시, point가 null이 아니고 amount가 0보다 크면 성공한다.")
    void testUsePointCommand_Success() {
        ...
        // when
        UsePointCommand command = new UsePointCommand(point, amount);
    ...
    @Test
    @DisplayName("UsePointCommand 생성 시, point가 null이면 PointNotExistError가 발생한다.")
    void testUsePointCommand_PointNotExistError() {
        ...
        Exception exception = assertThrows(PointNotExistError.class, () -> {
            new UsePointCommand(point, amount);
        });
